...
group by Month
 ...
 -- 7.Customers with the Highest Ride Frequency & Spending 
 
 select c.customer_id , c.customer_name , count(r.ride_id) as Total_rides , coalesce(sum(rp.amount_paid),0) as total_spent 
from customers as c inner join rides as r on c.customer_id = r.customer_id
...
order by total_spent desc , total_rides desc limit 10 ; 

--  8.  Identify drivers who earned the highest revenue.
...
-- 9 . Analyze the most common reasons for ride cancellations.
...
SELECT customer_id  , COUNT(*) AS ride_count FROM Rides 
GROUP BY customer_id ORDER BY ride_count DESC LIMIT 5;
...
FROM Ride_Payments GROUP BY payment_method
...
FROM drivers  GROUP BY driver_id ORDER BY avg_rating DESC LIMIT 5;
 
 --  15 .   HOW MANY RIDES WERE COMPLETED WITHOUT CUSTOMER RATINGS? 

SELECT COUNT(*) AS no_rating_rides FROM Rides r
...
-- 19 .What is the total revenue generate from Night-time rides .
...
-- 20.  How many customers have used Rapido in Mltiple cities .
...
SELECT c.gender, round(COUNT(r.ride_id) / COUNT(DISTINCT DATE_FORMAT(r.ride_date, '%Y-%m')),1)
AS avg_rides_per_month FROM Rides as r
JOIN Customers as c ON r.customer_id = c.customer_id
GROUP BY c.gender;

-- 22. Find Highest Spending Age Group .
